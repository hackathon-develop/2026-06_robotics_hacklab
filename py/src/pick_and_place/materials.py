...
MOTOR_RGBA: tuple[float, float, float, float] = (0.1, 0.1, 0.1, 1.0)
...
class MaterialConfig:
    ...
    # mesh name (no extension) → material name ('plastic', 'motor', or key in custom)
...
def apply_materials(spec: mujoco.MjSpec, config: MaterialConfig) -> None:
    ...
    for name in ("plastic", "motor", "collision"):
        if name in needed:
