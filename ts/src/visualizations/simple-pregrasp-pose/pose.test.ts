...
describe('simple pregrasp pose', () => {
  it.each(SIDE_FACES)('keeps the gripper vertical for %s', face => {
    const matrix = createSimplePregraspMatrix(face, DEFAULT_CUBE_POSE);
    expect(matrix).toBeDefined();
    if (!matrix) { throw new Error('Expected a valid simple pregrasp pose'); }
  ...
  it('allows yaw around the vertical axis', () => {
    const matrix = createSimplePregraspMatrix('+x', {
      ...DEFAULT_CUBE_POSE,
      yaw: Math.PI / 3
    });
  ...
  it('has no solution when the selected face is tilted', () => {
    expect(createSimplePregraspMatrix('+x', {
      ...DEFAULT_CUBE_POSE,
      pitch: 0.1
    })).toBeUndefined();
    expect(createSimplePregraspMatrix('+y', {
      ...DEFAULT_CUBE_POSE,
      roll: 0.1
    })).toBeUndefined();
  });
